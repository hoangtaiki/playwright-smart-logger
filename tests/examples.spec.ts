...
test.describe('SmartLog - Examples & Integration Tests', () => {
  test.describe('Real-world Usage Patterns', () => {
    test('basic logging that appears only on failure', async ({
      page,
      smartLog,
    }) => {
    ...
    test('assertion logging for inline validation', async ({
      page,
      smartLog,
    }) => {
      await page.goto('https://example.com');
      ...
      smartLog.assert(
        page.url().includes('example'),
        'URL should contain "example"'
      );
    ...
    test('counter usage for iterative operations', async ({
      page,
      smartLog,
    }) => {
      await page.goto('https://example.com');
  ...
  test.describe('Error Scenarios', () => {
    test('failing test to demonstrate log output', async ({
      page,
      smartLog,
    }) => {
      test.fail();
  ...
  test.describe('Integration with Page Objects', () => {
    class TestPage {
      ...
      async validateTitle(expectedPattern: RegExp) {
        ...
        this.smartLog.assert(
          matches,
          `Title should match pattern: ${expectedPattern}`
        );
